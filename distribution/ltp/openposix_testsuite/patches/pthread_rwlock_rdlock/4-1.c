...
int main(void)
{
	...
	if (thread_state == ENTERED_THREAD) {
		...
		do {
			sleep(1);
		} while (thread_state != EXITING_THREAD && cnt++ < 6);
		if (thread_state == ENTERED_THREAD) {
			printf("sig_thread is still blocking after %d more seconds!\n", cnt);
			system("ps -T | grep pthread");
		}
		else if (thread_state == EXITING_THREAD) {
			printf("sig_thread exited %d after about %d more seconds\n", thread_state, cnt);
		}
		else if (thread_state != EXITING_THREAD) {
			printf("Unexpected thread state: %d\n", thread_state);
		}
