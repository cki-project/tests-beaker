...
{
    rlLog "Aborting current task because $*"
...
{
    rlLog "Skipping current task because $*"
...
function cki_report_result
{
    ...
    typeset argv="$*"
    case $rc in
    $CKI_PASS) rlPass $argv ;;
    $CKI_FAIL) rlFail "$g_reason_fail #$argv#" ;;
    $CKI_UNSUPPORTED) cki_skip_task  "$g_reason_unsupported #$argv#" ;;
    $CKI_UNINITIATED) cki_abort_task "$g_reason_uninitiated #$argv#" ;;
    *) ;;
    esac
}
